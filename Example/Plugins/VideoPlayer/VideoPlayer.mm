...
-(void) setup{	
	...
	dispatch_async(dispatch_get_main_queue(), ^{	
		...
		NSString * basePath = [@"~/Desktop/Parsifal/" stringByExpandingTildeInPath];
		for(int i=0;i<NUMVIDEOS;i++){
			NSString * fileNumber = [NSString stringWithFormat:@"%i",i+1];
			
			[dict setObject:[NSString stringWithFormat:@"%@/Parsifal%@.mov",basePath,fileNumber] forKey:QTMovieFileNameAttribute];
			...
			if(error != nil){ 
				...
				[loadedFilesController addObject:[NSDictionary dictionaryWithObjectsAndKeys:
												  [NSNumber numberWithInt:i],@"number",
												  [NSString stringWithFormat:@"Parsifal%@.mov",fileNumber],@"name",
